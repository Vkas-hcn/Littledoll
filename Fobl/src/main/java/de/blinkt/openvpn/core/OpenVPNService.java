...
//            nbuilder.setContentTitle(getString(R.string.notifcation_title_notconnect));
        if (mProfile != null) nbuilder.setContentTitle(mProfile.mName);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            String notificationId = channel;
            NotificationChannel noch = new NotificationChannel(notificationId, notificationId, NotificationManager.IMPORTANCE_HIGH);
            NotificationManager manager = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
            if (manager.getNotificationChannel(notificationId) == null) {
                manager.createNotificationChannel(noch);
            }
        }
...
//        addVpnActionsToNotification(nbuilder);
        ...
        if (runningOnAndroidTV() && !(priority < 0)) guiHandler.post(() -> {
            ...
            String name;
            if (mProfile != null) name = mProfile.mName;
            else name = "OpenVPN";
//                String toastText = String.format(Locale.getDefault(), "%s - %s", name, msg);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        showNotification(VpnStatus.getLastCleanLogMessage(this), VpnStatus.getLastCleanLogMessage(this), NOTIFICATION_CHANNEL_NEWSTATUS_ID, 0, ConnectionStatus.LEVEL_START, null);
    ...
    private VpnProfile fetchVPNProfile(Intent intent) {
        ...
        String name = "(null)";
        if (mProfile != null) name = mProfile.getName();
    ...
    private void startOpenVPN(Intent intent, int startId) {
        ...
        guiHandler.post(() -> {
            if (oldDeviceStateReceiver != null) unregisterDeviceStateReceiver(oldDeviceStateReceiver);
    ...
    private OpenVPNManagement instantiateOpenVPN3Core() {
        ...
        } catch (IllegalArgumentException | InstantiationException | InvocationTargetException | NoSuchMethodException | ClassNotFoundException |
    ...
    public ParcelFileDescriptor openTun() {
        ...
        if ("samsung".equals(Build.BRAND) && mDnslist.size() >= 1) {
            ...
            try {
                ...
                if (!mDnslist.get(0).contains(":")) VpnStatus.logError("Error parsing DNS Server IP: " + mDnslist.get(0));
            }
        }
...
//        }
    ...
    public void updateState(String state, String logmessage, int resid, ConnectionStatus level, Intent intent) {
...

        {
            ...
            // CONNECTED
            // Does not work :(
            showNotification(VpnStatus.getLastCleanLogMessage(this), VpnStatus.getLastCleanLogMessage(this), channel, 0, level, intent);
    ...
    public void updateByteCount(long in, long out, long diffIn, long diffOut) {
        if (mDisplayBytecount) {
            Intent intent = new  Intent("com.speed.b");
            intent.putExtra("download",in);
            intent.putExtra("upload",out);
